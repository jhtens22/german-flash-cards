    ...
    <div className="vbox">
      ...
      <nav>
        <ul>
          {
            vocabs.map((g, ndx) => {
              return <li key={ndx}><Link to={"/vocab/" + ndx}></Link>{g.group}</li>
            })
          }
        </ul>
      </nav>
...
export function Group(props) {
  ...
  return (
    <div className="gbox">
      ...
      <nav>
        <ul>
          <li><Link to="/vocab"></Link>Main Page</li>
          <li><Link to={"/vocab/" + gid + "/word/0"}></Link>Start Here</li>
        </ul>
      </nav>
    </div>
  )
    ...
    <div className="wbox">
      ...
      <nav>
        <ul>
          ...
          {
            (+wid) - 1 >= 0
            ? <li><Link to={"/vocab/" + gid + "/word/" + ((+wid) - 1)}></Link>&lt;&lt; Prev</li>
            :<></>
          }
          {
            (+wid) + 1 < vocabs[+gid].words.length
            ? <li><Link to={"/vocab/" + gid + "/word/" + ((+wid) - 1)}></Link>Next &gt;&gt;</li>
            :<></>
          }
        </ul>
      </nav>
